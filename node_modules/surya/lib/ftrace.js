...
function ftrace(functionId, accepted_visibility, files) {
  ...
  var _loop = function _loop(file) {
    ...
    parser.visit(ast, {
      ...
      CustomErrorDefinition: function CustomErrorDefinition(node) {
        functionsPerContract[contractName].push(node.name);
  ...
  function constructPerFileFunctionCallTree(ast) {
    ...
    parser.visit(ast, {
      ...
      FunctionCall: function FunctionCall(node) {
        ...
        // START BLOCK
        if (parserHelpers.isRegularFunctionCall(node, contractNames, eventsOfDependencies, structsOfDependencies, customErrorNames)) {
          name = expr.name;
